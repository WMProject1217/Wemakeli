...
$readinfofile = @ fopen("./$readlabel/info.wmst", "r") or die("<title>Error 0x00000002</title>Error 0x00000002<br>Read info load unsuccessful.");
$title = fgets($readinfofile);
$outputtime = fgets($readinfofile);
$uploadmaster = fgets($readinfofile);
$readnumber= fgets($readinfofile);
fclose($readinfofile);
$countfile = @ fopen("./$readlabel/count.wmst", "r");
$playnumber=fgets($countfile);
$playnumber = $playnumber + 1;
fclose($countfile);
$countfile = @ fopen("./$readlabel/count.wmst", "w");
...
if (isset($_COOKIE["username"])){
    ...
    echo "<td class='talk'>";
    echo "<div>评论区</div>";
    echo "<form action='./$readlabel/posttalk.php?read=$readlabel' method='POST'>";
    echo "<textarea style='OVERFLOW:  Visble' name='usertalk' value='' class='talkboxinput'></textarea>";
    echo "<input type='submit' value='提交'>";
    echo "</form>";
    @ include("./$readlabel/talk.php");
    echo "</td>";
    echo "</tr>";
    echo "</table>";
    echo "</td>";
